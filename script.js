let cityInput = document.getElementById("city_input"),
  ...
  api_key = "6ee3d8892dbdc9268cf3bd2e74328ef0",
  currentWatherCard = document.querySelectorAll(".weather-left .card")[0],
...
function getWeatherDetails(name, lat, lon, country, state) {
  ...
  fetch(FORECAST_API_URL)
    ...
    .then((data) => {
      ...
      document.querySelectorAll(".day-forecast").forEach((card) => {
        card.innerHTML = "";
      });
  ...
  navigator.geolocation.getCurrentPosition(
    (position) => {
      ...
      fetch(REVERSE_GEOCODING_URL)
        ...
        .catch(() => {
          alert("Failed to fetch user coordinate");
        });
    },
    (error) => {
      if (error.code === error.PERMISSION_DENIED) {
        alert(
          "Geolocation permisiion denied. Please reset location permission to grant access again"
        );
      }
    }
  );
